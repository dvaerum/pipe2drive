...
fn select_path(file: Option<&str>, default_path: &str) -> String {
    let path = misc::config_file(file, default_path).to_string_lossy().to_string();
    path
}
...
async fn read_client_secret(file: Option<&str>) -> ApplicationSecret {
    ...
    let var_name = read_application_secret(
        &client_secret_path
    ).await;
...
pub async fn auth(client_secret_file: Option<&str>, client_token_file: Option<&str>) -> HubType {
    ...
    let auth_result = InstalledFlowAuthenticator::builder(
        secret.await, InstalledFlowReturnMethod::Interactive)
